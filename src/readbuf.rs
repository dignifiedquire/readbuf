//! Based on https://github.com/rust-lang/rust/blob/master/library/std/src/io/readbuf.rs @ e012a191d768adeda1ee36a99ef8b92d51920154
...
impl<'a> ReadBuf<'a> {
    ...
    pub fn new(buf: &'a mut [u8]) -> ReadBuf<'a> {
        ...
        ReadBuf {
            ...
            buf: unsafe { as_uninit_slice_mut(buf as *mut [u8]).unwrap() },
            filled: 0,
            initialized: len,
        }
    }
...
#[inline]
unsafe fn as_uninit_slice_mut<'a, T>(s: *mut [T]) -> Option<&'a mut [MaybeUninit<T>]> {
    if s.is_null() {
        None
    } else {
        // SAFETY: the caller must uphold the safety contract for `as_uninit_slice_mut`.
        Some(core::mem::transmute::<*mut [T], &'a mut [MaybeUninit<T>]>(
            s,
        ))
    }
}
...
fn write_slice<'a, T>(this: &'a mut [MaybeUninit<T>], src: &[T]) -> &'a mut [T]
where
    T: Copy,
{
